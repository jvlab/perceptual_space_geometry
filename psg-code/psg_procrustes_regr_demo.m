...
if ~exist('nshuff_resids') nshuff_resids=100; end
...
max_dim=getinp('maximum dimension to use','d',[min_dim 7],max_dim);
results=struct;
if (if_builtin)
...
results.dim_names={'iset','jset','idim','shuffles'};
results.procrustes=struct();
results.affine=struct();
results.perspective=struct();
%
for iset=1:nsets-1
    for jset=iset+1:nsets
        for ijrev=1:2
            ...
            for idim=min_dim:max_dim
                ...
                %
                if (nshuff_proc>0) & iset==1 & jset==2 & ijrev==1
                    perms_proc=zeros(nshuff_proc,npts); %set up a consistent set of shuffles across dimensions and dataset pairs
                ...
                if nshuff_proc>0
                    ...
                    results.procrustes.d_shuff(ref_ptr,adj_ptr,idim,:)=reshape(d_shuff_proc,[1 1 1 nshuff_proc]);
                end
                %
                ...
                if (idim==2) | (idim==3)
                    ...
                    switch idim
                        case 2
                            ...
                            hold on;
                            hac=plot(adj_check(:,1),adj_check(:,2),'rx','MarkerSize',6);
                            hap=plot(adj_proc(:,1),adj_proc(:,2),'g.','MarkerSize',8);
                            har1=plot(adj_xform{1}(:,1),adj_xform{1}(:,2),'m.','MarkerSize',8);
                            har2=plot(adj_xform{2}(:,1),adj_xform{2}(:,2),'c.','MarkerSize',8);
                            for k=1:npts
                                plot([ref(k,1) adj_proc(k,1)],[ref(k,2) adj_proc(k,2)],'g');
                                plot([ref(k,1) adj_xform{1}(k,1)],[ref(k,2) adj_xform{1}(k,2)],'m');
                                plot([ref(k,1) adj_xform{2}(k,1)],[ref(k,2) adj_xform{2}(k,2)],'c');
                            end
                        case 3
                            hr=plot3(ref(:,1),ref(:,2),ref(:,3),'k.','MarkerSize',8);
                            hold on;
                            hac=plot3(adj_check(:,1),adj_check(:,2),adj_check(:,3),'rx','MarkerSize',6);
                            hap=plot3(adj_proc(:,1),adj_proc(:,2),adj_proc(:,3),'g.','MarkerSize',8);
                            har1=plot3(adj_xform{1}(:,1),adj_xform{1}(:,2),adj_xform{1}(:,3),'m.','MarkerSize',8);
                            har2=plot3(adj_xform{2}(:,1),adj_xform{2}(:,2),adj_xform{2}(:,3),'c.','MarkerSize',8);
                            for k=1:npts
                                plot3([ref(k,1) adj_proc(k,1)],[ref(k,2) adj_proc(k,2)],[ref(k,3) adj_proc(k,3)],'g');
                                plot3([ref(k,1) adj_xform{1}(k,1)],[ref(k,2) adj_xform{1}(k,2)],[ref(k,3) adj_xform{1}(k,3)],'m');
                                plot3([ref(k,1) adj_xform{2}(k,1)],[ref(k,2) adj_xform{2}(k,2)],[ref(k,3) adj_xform{2}(k,3)],'c');
                            end
                            view(3);
                            zlabel('dim 3');
                    end
