...
%
% See also: DOMDS, MIN_NBR_CONN, DISTANCES, COOTODSQ, RIEMDS_ISOMAP_DEMO,
%  DOMDS, MIN_NBR_CONN, DISTANCES, COOTODSQ, RIEMDS_ISOMAP_PLOT.
%    
%
...
if ~exist('opts_nn') opts_nn=struct(); end %for min_nbr_conn
%
...
opts_nn=filldefault(opts_nn,'if_log',0);
%
...
for iset=1:nsets
    for dim_ptr=1:length(dim_list_anal);
        ...
        for nn_ptr=1:length(nn_list)
            nn=nn_list(nn_ptr);
            %determine the minimum distance for which each point has nn neighbors
            [nbr_mindist(dim_anal,nn,iset),nbr_mtx,nbr_graph,opts_used]=min_nbr_conn(dists,setfield(opts_nn,'min_nn',nn));
            nbr_mtx_nonzero(dim_anal,nn,iset)=(sum(nbr_mtx(:)>0)/2);
            isomap_dists=distances(nbr_graph);
            eivals_raw=domds(isomap_dists,1); % [eival,eivec]=domds(distmtx,p) does a multidimensional scaling
            [eivals_sort,ix]=sort(abs(eivals_raw),'descend');
            eivals_raw=eivals_raw(ix);
            % Note: The eigenvalues need to be divided by 2, prior to square root, to obtain coords that recapitulte the distances.
            eivals_iso{dim_anal,nn,iset}=real(eivals_raw)/2;
        end %nn_ptr
        eivals_raw=domds(dists,1);
        [eivals_sort,ix]=sort(abs(eivals_raw),'descend');
        eivals_raw=eivals_raw(ix);
        eivals_std{dim_anal,1,iset}=real(eivals_raw)/2;
    end %dim_ptr
end %iset
%
%plot
