function pipeline=psg_coord_pipe_util(typename,opts,sets,file_list,sets_combined)
% pipeline=psg_coord_pipe_util(typename,opts,sets,file_list,sets_combined) is a
% utility to create the pipeline field.
%
% each of these input variables becomes the field of pipeline, with the
%   same name, except that 'type' is the field name for typename
