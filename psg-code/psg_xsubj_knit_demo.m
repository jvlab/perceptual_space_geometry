...
% 30Nov25: add dim_aug, and within-subject mode
%
%  See also:  RS_READ_COORDSETS, RS_KNIT_COORDSETS, RS_KNIT_COORDSETS_DEMO, PSG_PCAOFFSET.
%
if_within=getinp('1 for within-subject mode','d',[0 1],0);
if if_within
    subj_string=getinp('subject ID, e.g., MC','s',[],'MC');
    ui_filter=cat(2,'./psg_data/','*3pt_coords_',subj_string,'_sess01_10.mat');
    coord_string=[];
    coord_subj_string=cat(2,'subject: ',subj_string);
else
    coord_string=getinp('coordinate string, e.g., bgca3pt','s',[],'bgca3pt');
    ui_filter=cat(2,'./psg_data/',coord_string,'_coords_*_sess01_10.mat');
    subj_string=[];
    coord_subj_string=cat(2,'coords: ',coord_string);
end
...
for k=1:length(data_align.sets)
    ...
    subj_ids{k}=subj_id;
    coord_id=data_align.sets{k}.paradigm_name;
    coord_ids{k}=coord_id;
    data_align.sets{k}.label=cat(2,coord_id,':',subj_id);
    ...
    if ~contains(coord_list_string,coord_id)
        if ~isempty(coord_list_string) 
            coord_list_string=cat(2,coord_list_string,'+');
        end
        coord_list_string=cat(2,coord_list_string,coord_id);
    end
end
%knit
[data_knit,aux_knit_out]=rs_knit_coordsets(data_align,aux_knit);
set(gcf,'Name',sprintf('%s (%s): no scaling',coord_list_string,subj_id_string));
...
set(gcf,'Name',sprintf('%s (%s): scaling+norm',coord_list_string,subj_id_string));
set(gcf,'Position',[50 100 1500 700]);
axes('Position',[0.25,0.04,0.01,0.01]); %for text
text(0,0,sprintf('if_pca: %1.0f',if_pca),'Interpreter','none','FontSize',8);
axis off;
...
r.nsets=nsets;
r.if_pca=if_pca;
r.subj_string=subj_string; %subject string as entered
r.subj_id_string=subj_id_string; %found across files
r.subj_ids=subj_ids; %subject for each file
r.coord_string=coord_string;
r.coord_list_string=coord_list_string;
r.coord_ids=coord_ids;
r.nshuffs=nshuffs;
