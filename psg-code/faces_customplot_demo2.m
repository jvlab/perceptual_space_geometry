%faces_customplot_demo2: demonstrate custom plotting of btcsel data from tables
%
% streamlined version of faces_customplot_demo, only plots 'flip_any', surrogates,
% only group 1, different axis settings and spacing settings to match
% bright_customplot_demo2
%
%   See also: PSG_LIKE_ANALTABLE, BTCSEL_CUSTOMPLOT_DEMO, BTCSEL_CUSTOMPLOT_DEMO2, FACES_CUSTOMPLOT_DEMO.
%
...
opts_plot_def=filldefault(opts_plot_def,'abscissa_para_space',0.3);
opts_plot_def=filldefault(opts_plot_def,'abscissa_subj_space',0.05);
...
for if_afixed=-1:-1 %just plot with a not fixed
    ...
    for ig=1:length(pgroups)
        ...
        if if_afixed~=0
            ...
            opts_plot.abscissa_para_space=0.15;
