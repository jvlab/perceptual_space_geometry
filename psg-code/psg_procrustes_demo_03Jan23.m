...
% Compares one coordinate set with another
%
%   To do:
% * Graphical output for
%    * fit as number of dimensions
%    * 3d plot of coords after alignment
% * If two or more datasets present: consensus coordinates
%
...
%
%basic analysis: one set with itself and pairwise analyses
%
    ...
    if (if_ok==0)
        ...
        if ~isempty(dims_avail)
            ...
            while if_ok_plot==0
                ...
                lib_list=getinp('choice(s), 0 to end and choose other datasets or model dimensions','d',[0 4]);
                if min(lib_list)==0
                    if_ok_plot=1;
                else
                    dm=cell(0);
