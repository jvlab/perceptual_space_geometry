...
if (opts.if_log)
    ...
    %tallies
    %
    disp('number of triplets with at least the given number of repeats per arm');
    disp(cat(2,'         ',sprintf(' %5.0f',[0:umi_stats.combined.umi_least_max])));
    for isess=1:opts.cond_nsess
        disp(cat(2,sprintf('sess %4.0f',isess),sprintf(' %5.0f',umi_stats.eachsess.umi_notshown(1,isess),umi_stats.eachsess.umi_least{1,isess})));
    end
    if (opts.if_cumulative)
        for isess=1:opts.cond_nsess
            disp(cat(2,sprintf('sess 1-%2.0f',isess),sprintf(' %5.0f',umi_stats.cumulative.umi_notshown(1,isess),umi_stats.cumulative.umi_least{1,isess})));
        end
    end
    disp(cat(2,' combined',sprintf(' %5.0f',umi_stats.combined.umi_notshown,umi_stats.combined.umi_least)));
    %
    disp('number of triplets with at most  the given number of repeats per arm');
    disp(cat(2,'         ',sprintf(' %5.0f',[0:umi_stats.combined.umi_most_max])));
    for isess=1:opts.cond_nsess
        disp(cat(2,sprintf('sess %4.0f',isess),sprintf(' %5.0f',umi_stats.eachsess.umi_notshown(1,isess),umi_stats.eachsess.umi_most{1,isess})));
    end
    if (opts.if_cumulative)
        for isess=1:opts.cond_nsess
            disp(cat(2,sprintf('sess 1-%2.0f',isess),sprintf(' %5.0f',umi_stats.cumulative.umi_notshown(1,isess),umi_stats.cumulative.umi_most{1,isess})));
        end
    end
    disp(cat(2,' combined',sprintf(' %5.0f',umi_stats.combined.umi_notshown,umi_stats.combined.umi_most)));
    %
    disp('number of triplets with a total of the given number of repeats');
    disp(cat(2,'         ',sprintf(' %5.0f',[0:umi_stats.combined.umi_total_max])));
    for isess=1:opts.cond_nsess
        disp(cat(2,sprintf('sess %4.0f',isess),sprintf(' %5.0f',umi_stats.eachsess.umi_notshown(1,isess),umi_stats.eachsess.umi_total{1,isess})));
    end
    if (opts.if_cumulative)
        for isess=1:opts.cond_nsess
            disp(cat(2,sprintf('sess 1-%2.0f',isess),sprintf(' %5.0f',umi_stats.cumulative.umi_notshown(1,isess),umi_stats.cumulative.umi_total{1,isess})));
        end
    end
    disp(cat(2,' combined',sprintf(' %5.0f',umi_stats.combined.umi_notshown,umi_stats.combined.umi_total)));
end
opts_used=opts;
return
%
...
counts_least=min(triplet_table(:,[4:6]),[],2); 
...
counts_most=max(triplet_table(:,[4:6]),[],2); 
...
%how many triplets have at total given number of trials for each arm
counts_total=sum(triplet_table(:,[4:6]),2); 
