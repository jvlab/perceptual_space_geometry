...
% 
%   See also:  PSG_SESSION_STATS, PSG_SETUP_DEMO, PSG_COND_WRITE, PSG_SESSCONFIG_MAKE, PSG_DEFOPTS, PSG_UMI_STATS.
if (nargin<2)
    opts=struct;
end
...
if (opts.if_log)
    ...
    disp('number of triads with a given number of repeats');
    disp(cat(2,'         ',sprintf(' %5.0f',[0:triad_count_max])));
    for isess=1:opts.cond_nsess
        disp(cat(2,sprintf('sess %4.0f',isess),sprintf(' %5.0f',triad_stats.eachsess.triads_notshown(1,isess),triad_stats.eachsess.triad_repeats{1,isess})));
    end
    if (opts.if_cumulative)
        for isess=1:opts.cond_nsess
            disp(cat(2,sprintf('sess 1-%2.0f',isess),sprintf(' %5.0f',triad_stats.cumulative.triads_notshown(1,isess),triad_stats.cumulative.triad_repeats{1,isess})));
        end
    end
    disp(cat(2,' combined',sprintf(' %5.0f',triad_stats.combined.triads_notshown,triad_stats.combined.triad_repeats)));
end
return
%
