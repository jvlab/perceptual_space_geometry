...
%
% streamlined version of bright_customplot_demo, only plots 'flip_any',
% only frac_keep=0.5
% surrogates, different axis settings
%
%   See also: PSG_LIKE_ANALTABLE, BTCSEL_CUSTOMPLOT_DEMO, FACES_CUSTOMPLOT_DEMO, PSG_TYPENAMES2COLORS,
%      BRIGHT_CUSTOPLOT_DEMO.
%
dirichlet_range=[0.00 0.25];
%
...
opts_plot_def=filldefault(opts_plot_def,'frac_keep_choices',[2]); %fraction to keep
opts_plot_def=filldefault(opts_plot_def,'box_halfwidth',0.012*diff(dirichlet_range)/1.25); %rescale box half-width to match abscissa
opts_plot_def=filldefault(opts_plot_def,'abscissa_alt',0);
opts_plot_def=filldefault(opts_plot_def,'abscissa_para_space',1.0);
opts_plot_def=filldefault(opts_plot_def,'abscissa_subj_space',0.12);
...
pgroups{1}.list_suff={'_cXs','_c01','_c02','_s01Xs03Xs05Xs07','_s02Xs04Xs06Xs08'};
pgroups{1}.names={'all','c01','c02','s1357','s2468'};
pgroups{1}.colors=[0.00 0.00 0.00;round(colors_def.b);colors_def.c;0.25 0.75 0.00;0.50 0.50 0.00]; %for consistency with colors in btcsel and faces custom plot
pgroups{1}.abscissa_para_order=[1 2 3 4 5];
%
