...
% PSG_SPEC2FILENAME, BTC_LETCODE2VEC, PSG_SPOKES_SETUP, BTC_QUAD_BGCA_MAKE.
%
fn_prompt='file name, typically btc_quad_bcde*.mat';
%
if ~exist('ndigs') ndigs=4; end %number of digits in file name
mix_scenarios={{'bc','de'},{'bd','ce'},{'b','c','d','e'},{'bc','de'}};
if_compensates=[0 0 0 1]; %whether to compensate for induced correlations
%
dict=btc_define;
codel=dict.codel;
nbtc=length(codel);
...
if if_read==0
    ...
    for iq=1:2^n4
        ...
        comp_vals=repmat(bc_target*ncomponents*quad_signs(iq,:),[ncomponents 1]);
        ...
        r.spec_final{iq}=spec_final; %specify b,c,d,e
