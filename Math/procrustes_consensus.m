...
%   initialize_set: 1 to nsets, which set to initialize with
%
...
niters=0;
rms=Inf;
consensus=z(:,:,opts_pcon.initialize_set); %initialize guess of consensus to equal one set
while (niters<opts_pcon.max_niters & rms>opts_pcon.max_rmstol)
    ...
    %realign to the starting dataset
    [d,consensus_new]=procrustes(z(:,:,opts_pcon.initialize_set),consensus_new,'Scaling',scaling_token,'Reflection',reflection_token);
